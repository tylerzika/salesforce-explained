...
**Health Test Data** app whose page renders the Health Cloud data model as a
schema diagram — each object is a node listing its key fields, wired to the
others by its real foreign keys, with a live record count and a button that
generates a record into it. Same engine as `test-data-sales` (Apex → Models API
→ insert), and like it, this package depends only on `models`.

The diagram covers four objects and the relationships between them:

| Node | Object | Generated by |
|---|---|---|
| Patient | `Account` (Person Account) | Models API |
| Healthcare Provider | `HealthcareProvider` (`AccountId` → Account) | Models API |
| Care Program | `CareProgram` (`SponsorId` → Account) | Models API |
| Care Program Enrollee | `CareProgramEnrollee` (`AccountId` → Account, `CareProgramId` → CareProgram) | deterministic link |

`CareProgramEnrollee` is the enrollment junction, so its factory takes no model
call — the meaningful content of a junction row is its two foreign keys, not
invented prose. It requires an existing Patient and Care Program and says so if
either is missing.
...
sf package install -p test-data-health@0.1.0-2 -o health -w 10
